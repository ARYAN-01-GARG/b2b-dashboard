...
interface BiLineCardProps {
    title: string;
    yLabel: string;
    series: Series[];
}

function BiLineCard({ title, yLabel, series }: BiLineCardProps) {
    const chartData = series[0]?.data.map((p, idx) => ({
        name: p.name,
        [series[0].name]: p.value,
        ...(series[1] ? { [series[1].name]: series[1].data[idx]?.value ?? 0 } : {}),
    })) ?? [];
        ...
        <div className="bg-white rounded-2xl border-2 border-[#A0A0A042] shadow p-6 w-full">
            ...
            <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData} margin={{ top: 8, right: 16, left: 0, bottom: 24 }} className="cursor-pointer">
